...
struct headers {
	ethernet_t ethernet;
	form_t form;
	header_stack<gradient_t, MAX>  gradient;
}
...
// ========== Registers ==========
register<bit<32>>(MAX) gradient_sum;
...
parser MyParser(packet_in pkt,
	...
	state parse_grad{
		transition select(meta.next_slot < meta.grad_count){
			true: check_grad;
			default: accept;
		}
	}
	...
	state check_grad{
		...
		meta.next_slot = meta.next_slot + 1;
...
control MyDeparser(packet_out pkt, in headers hdr) {
	...
	for (bit<32> i = 0; i < meta.grad_count; i = i + 1) {
	pkt.emit(hdr.gradient[i]);
	}
    }
}
